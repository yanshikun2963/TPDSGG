...
def build_dataset(cfg, dataset_list, transforms, dataset_catalog, is_train=True):
    ...
    for dataset_name in dataset_list:
        ...
        # ==== PMR Relabel Hook ====
        if is_train and hasattr(cfg.MODEL, 'ROI_RELATION_HEAD'):
            use_pmr = getattr(cfg.MODEL.ROI_RELATION_HEAD, 'USE_PMR_RELABEL', False)
            pmr_file = getattr(cfg.MODEL.ROI_RELATION_HEAD, 'PMR_RELABEL_FILE', "")
            if use_pmr and pmr_file and hasattr(dataset, 'set_relabel_file'):
                dataset.set_relabel_file(pmr_file)
