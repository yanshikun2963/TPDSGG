...
class VGDataset(torch.utils.data.Dataset):
    ...
    def set_relabel_file(self, relabel_file):
        """Load PMR relabeled labels and overwrite self.relationships."""
        import pickle
        import os
        if not relabel_file or not os.path.exists(relabel_file):
            print(f"[VG-PMR] No relabel file found at: {relabel_file}")
            return
        print(f"[VG-PMR] Loading relabel from: {relabel_file}")
        with open(relabel_file, 'rb') as f:
            data = pickle.load(f)
        relabel_map = data['relabel_map']
        n_replaced = 0
        n_total = len(self.relationships)
        for idx in range(n_total):
            if idx in relabel_map:
                new_triplets = relabel_map[idx]
                old = self.relationships[idx]
                if old.shape[0] == new_triplets.shape[0]:
                    self.relationships[idx] = new_triplets.copy().astype(old.dtype)
                    n_replaced += 1
        print(f"[VG-PMR] Replaced {n_replaced}/{n_total} images")
        print(f"[VG-PMR] Internal: {data.get('internal_count', '?')}, "
