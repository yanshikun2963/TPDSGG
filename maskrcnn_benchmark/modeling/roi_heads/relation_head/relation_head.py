...
class ROIRelationHead(torch.nn.Module):
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        _pred_out = self.predictor(proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger)
        if len(_pred_out) == 4:
            refine_logits, relation_logits, add_losses, add_data = _pred_out
        else:
            refine_logits, relation_logits, add_losses = _pred_out
            add_data = {}
