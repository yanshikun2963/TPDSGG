"""Build the joint (subject, object, predicate) frequency table for VG-150.

Reads:  datasets/vg/VG-SGG-with-attri.h5
Writes: datasets/vg/soo_freq.npy, with shape (151, 151, 51) and dtype float32.

The ``relationships`` field in ``VG-SGG-with-attri.h5`` stores global box
indices, so we index ``labels`` directly without adding ``img_to_first_box``.
...
VG_DATA = "datasets/vg/VG-SGG-with-attri.h5"
OUTPUT = "datasets/vg/soo_freq.npy"
...
def main():
    ...
    predicates = f["predicates"][:].flatten()
    ...
    for img_idx in train_imgs:
        ...
        for rel_idx in range(rfirst, rlast + 1):
            s_global, o_global = relationships[rel_idx]
    ...
    print(
        f"[stats] non-zero cells: {(freq > 0).sum()} / {freq.size} "
        f"({(freq > 0).mean() * 100:.3f}%)"
    )
    print(f"[stats] max freq: {freq.max():.0f}, total: {freq.sum():.0f}")

    pred_totals = freq.sum(axis=(0, 1))[1:]
    print(
        f"[stats] predicate freq range: {pred_totals.min():.0f} - {pred_totals.max():.0f}"
    )
